...
export default function ContractorDashboardScreen() {
  ...
  useEffect(() => {
    ...
    if (!uid) return
    // Listen to leads assigned to this contractor
    const q = query(
      collection(db, 'leads'),
  ...
  const inProgress   = leads.filter(l => l.status === 'quoted' || l.status === 'in_progress')
            ...
            <View key={lead.id} style={[s.leadCard, lead.status === 'new' && s.leadCardNew]}>
              ...
              {lead.status === 'new' && (
                <View style={s.leadBtns}>
                  <TouchableOpacity style={s.btnAccept}>
                    <Text style={s.btnAcceptText}>Accept Lead</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={s.btnDecline}>
...
const s = StyleSheet.create({
  ...
  stormBanner:  { backgroundColor: 'rgba(224,123,42,0.12)', borderWidth: 1, borderColor: 'rgba(224,123,42,0.4)', borderRadius: 14, padding: 14, marginBottom: 20 },
