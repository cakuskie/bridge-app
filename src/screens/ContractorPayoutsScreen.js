...
export default function ContractorPayoutsScreen() {
  ...
  useEffect(() => {
    ...
    if (!uid) return
    const q = query(collection(db, 'leads'), where('contractorId', '==', uid), where('status', '==', 'complete'), orderBy('createdAt', 'desc'))
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        {loading ? (
          ...
          <View style={s.emptyCard}>
            <Ionicons name="cash-outline" size={36} color={MUTED} style={{ marginBottom: 12 }} />
...
const s = StyleSheet.create({
  heading:      { fontSize: 26, fontWeight: '700', color: WHITE, marginBottom: 20 },
  ...
  summaryValue: { fontSize: 16, fontWeight: '700', color: WHITE },
  ...
  promoTitle:   { fontSize: 13, fontWeight: '700', color: GREEN },
  ...
  payoutName:   { fontSize: 13, fontWeight: '700', color: WHITE },
