    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: NAVY }}>
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        {loading ? (
          ...
          <View key={i} style={[s.card, a.desc !== 'No Active Alerts' && s.cardActive]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
              <View style={[s.badge, a.desc !== 'No Active Alerts' && s.badgeActive]}>
                <Text style={[s.badgeText, a.desc !== 'No Active Alerts' && s.badgeTextActive]}>
                  {a.desc !== 'No Active Alerts' ? `${a.icon} ${a.severity ?? 'Advisory'}` : '✅ Clear'}
                </Text>
              </View>
            </View>
